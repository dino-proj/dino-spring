...
public class OssAutoconfig {
  ...
  MinioOssService minioOssService(OssModuleProperties ossProps) {
    return new MinioOssService(ossProps.getMinio());
  }

  @Bean
  @ConditionalOnProperty(prefix = OssModuleProperties.PREFIX, name = "local")
  LocalOssService localOssService(OssModuleProperties ossProps) throws IOException {
  ...
  TencentOssService tencentOssService(OssModuleProperties ossProps) {
